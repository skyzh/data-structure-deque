...
namespace sjtu {
    ...
    private:
        ...
        template <typename U> void _destroy(U* head) {
            U* ptr = head;
            while (ptr) {
                U* tmp = ptr->next;
                delete ptr;
        ...
        template <typename U> U* remove_node(U* node) {
    ...
    private:
        Chunk* split_chunk(Chunk* chunk, Node* pos) {
            if (chunk->chunk_size > CHUNK_SIZE) {
                ...
                Chunk* right = new Chunk(split_node, chunk->tail, chunk->chunk_size - left->chunk_size, left, chunk->next);
                left->next = right;
        ...
        Chunk* merge_chunk(Chunk* left) {
            ...
            if (left->chunk_size + right->chunk_size <= CHUNK_SIZE / 2) {
                Chunk* chunk = new Chunk(left->head, right->tail, left->chunk_size + right->chunk_size, left->prev, right->next);
    ...
    public:
         ...
         */
        deque() { construct(); }
        deque(const deque &other) { construct(); copy_from(other); }
        /**
         * TODO Deconstructor
         */
        ~deque() { destroy(); }
        ...
        deque &operator=(const deque &other) {
            ...
            destroy(); construct(); copy_from(other);
            return *this;
        }
        /**
